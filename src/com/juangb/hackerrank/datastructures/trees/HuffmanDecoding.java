...
public class HuffmanDecoding {
     ...
     * Decoded String = "ABACA"
	   ...
	   void decode(String S ,Node root)
	    {
	        decodeRecursive(S,0,root,root);

	       
	    }
	   
	   void decodeRecursive(String string, int position, Node node, Node root){
		   if(string==null || position>=string.length() || node==null)
			   return;
		   if(string.charAt(position)=='0'){
			   if(node.left!=null){
				   //If it's a leaf print it's value and continue from the root
				   if(isLeaf(node.left)){					   
					   System.out.print(node.left.data);
					   decodeRecursive(string,position+1,root,root);
				   }else{
					   decodeRecursive(string,position+1,node.left,root);
				   }
			   }else{
			   ...
			   if(node.right!=null){
				   ...
				   if(isLeaf(node.right)){					   
					   ...
					   decodeRecursive(string,position+1,node.right,root);
				   }
			   }else{
				   System.out.println("Error!!");
			   }
